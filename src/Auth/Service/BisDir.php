...
class BisDir
{
     ...
     */
    public function __construct(PasswordHasherInterface $passwordHasher, string $hosts, string $baseDn, string $adminUsername, string $adminPassword, string $accountSuffix = '', int $port = 636, bool $followReferrals = true, bool $useTls = true, bool $useSsl = true)
    {
    ...
    public function syncPassword(String $email, string $password): BisDirResponse
    {
        ...
        if (null !== $entry) {
            $passwordEncoded = $this->passwordHasher->hash($password);
